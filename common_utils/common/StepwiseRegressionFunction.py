...
class evaluation:
    ...
    def evl_total(res_):
        ...
        res_abs = abs(res_)
        res_s_ = evaluation.evl_p(res_)
        SS_w = np.where((res_[:, 0] > res_s_[0, 0]) & (res_[:, 1] > res_s_[0, 1]) & (res_[:, 2] > res_s_[0, 2]) & (np.sum(res_abs, 1) != np.inf) & (~np.isnan(np.sum(res_abs, 1))))[0]
        res_ = res_[SS_w, :]
...
class GroupContribution:
    ...
    # g is group
    # =============================================================================
    def GroupSelectionExternal(regr, evl, g_train, g_test, g_s, Y_train, Y_test, ii_r, ii_c, XT_train, XT_test, XT0_train, XT0_test, R, Cg):
        sta = getattr(statistic, evl)
        reg = getattr(regress, regr)
        cal = getattr(calculation, regr)
...
# =============================================================================
    ...
    # ForwardExternal: forward stepwise regression evaluated by external validation
    # =============================================================================
    def ForwardExternal(regr, kk_ad_all, evl, Y_train, Y_test, ii_r, ii_c, XT_train, XT_test, XT0_train, XT0_test, gs_train, gs_test, R, C):
        sta = getattr(statistic, evl)
        reg = getattr(regress, regr)
        cal = getattr(calculation, regr)
        for kk in range(0, kk_ad_all):
            ...
            bb = bb_[i_]
            # 增加定位因子后，对定位因子进行重新选择
            ii_r, bb= GroupContribution.EvaluatepositionExternal(regr,evl, Y_train, Y_test, ii_r, ii_c, XT_train, XT_test, XT0_train, XT0_test, gs_train, gs_test, R, C)
        return ii_r, ii_c, bb    

    # =============================================================================
    # ForwardExternal: forward stepwise regression evaluated by external validation
    # =============================================================================
    def EvaluatepositionExternal(regr, evl, Y_train, Y_test, ii_r, ii_c, XT_train, XT_test, XT0_train, XT0_test, gs_train, gs_test, R, C):
        sta = getattr(statistic, evl)
        reg = getattr(regress, regr)
        cal = getattr(calculation, regr)
        ii_r0=[]
    ...
    # BackwardExternal: backward stepwise regression evaluated by external validation
    # =============================================================================
    def BackwardExternal(regr, s, kk_de_all, evl, Y_train, Y_test, ii_r, ii_c, XT_train, XT_test, XT0_train, XT0_test, gs_train, gs_test, R, C):
        sta = getattr(statistic, evl)
        reg = getattr(regress, regr)
        cal = getattr(calculation, regr)
